...
class Room
  ...
  def self.create(name:, description:, price:, datefrom:, dateto:, userid:)
    result = Database.query(
      "INSERT INTO rooms (name, description, price, datefrom, dateto, userid)
      VALUES($1, $2, $3, $4, $5, $6)
      RETURNING roomid, name, description, price, datefrom, dateto, userid;",
      [name, description, price, datefrom, dateto, userid]
    )
    Room.new(
  ...
  def self.available(bookfrom, bookto)
    result = Database.query("SELECT * FROM rooms WHERE datefrom <= $1 and dateto >= $2;", 
[bookfrom, bookto])   
    result.map do |room|
      Room.new(roomid: room['roomid'], name: room['name'], description: room['description'], 
price: room['price'], datefrom: room['datefrom'], dateto: room['dateto'], userid: room['userid'])
    end
  end
